...
(function (a4) {
    ...
    let zusatz;
    let label;
    let basketBaumart = [a4.sort[0][0], "" + a4.sort[0][1]];
    let basketHalter = ["bitte Halter auswählen", "0"];
    let basketBeleuchtung = [a4.light[0][0], "" + a4.light[0][1]];
    let basketSchmuck = [];
    ...
    function createElements() {
        ...
        for (let i = 0; i < a4.artikel.length; i++) {
            if (a4.artikel[i].art == "Sorte") {
                let choice = document.createElement("option");
                choice.innerText = a4.artikel[i].bezeichnung;
                choice.id = "option" + i;
        ...
        for (let i = 0; i < a4.artikel.length; i++) {
            if (a4.artikel[i].art == "Halter") {
                ...
                label.innerText = a4.artikel[i].bezeichnung;
        ...
        for (let i = 0; i < a4.artikel.length; i++) {
            if (a4.artikel[i].art == "Beleuchtung") {
                let opt2 = document.createElement("option");
                opt2.innerText = a4.artikel[i].bezeichnung;
                opt2.id = "option2." + i;
        ...
        for (let i = 0; i < a4.artikel.length; i++) {
            if (a4.artikel[i].art == "Schmuck") {
                ...
                label2.innerText = a4.artikel[i].bezeichnung;
        ...
        for (let i = 0; i < a4.artikel.length; i++) {
            if (a4.artikel[i].art == "Lieferung") {
                ...
                label3.innerText = a4.artikel[i].bezeichnung + ", " + a4.artikel[i].preis + "\u20ac";
    ...
    function warenkorb(_event) {
        ...
        for (let i = 0; i < a4.artikel.length; i++) {
            ...
            if (target.value == a4.artikel[i].bezeichnung && target.id == "selectBaumart") {
                basketBaumart[0] = a4.artikel[i].bezeichnung;
                basketBaumart[1] = "" + a4.artikel[i].preis;
            }
            if (target.id == "radio2." + i) {
                basketHalter[0] = a4.artikel[i].bezeichnung;
                basketHalter[1] = "" + a4.artikel[i].preis;
            }
            if (target.id == "radio3." + i) {
                basketLieferung[0] = a4.artikel[i].bezeichnung;
                basketLieferung[1] = "" + a4.artikel[i].preis;
            }
            if (target.value == a4.artikel[i].bezeichnung && target.id == "selectBeleuchtung") {
                basketBeleuchtung[0] = a4.artikel[i].bezeichnung;
                basketBeleuchtung[1] = "" + a4.artikel[i].preis;
            }
            if (target.id == "check" + i || target.id == "stepper" + i) {
                basketSchmuck[i] = [a4.artikel[i].bezeichnung, "" + (a4.artikel[i].preis * parseInt(stepper[i].value))];
            }
        }
